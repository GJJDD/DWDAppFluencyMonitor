...
#import "DWDMachThreadBacktrace.h"
#import "UIDevice+afm_Hardware.h"



static NSString *const kMTAppFluencyLogFilesDirectory = @"DWDAppFluencyLogFilesDirectory";

dispatch_queue_t dwd_fluency_monitor_queue() {
    static dispatch_queue_t dwd_fluency_monitor_queue;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dwd_fluency_monitor_queue = dispatch_queue_create("com.dianwoda.DWDStastistics.dwd_fluency_monitor_queue", NULL);
    });
    return dwd_fluency_monitor_queue;
}

dispatch_queue_t dwd_fluency_monitor_logs_queue() {
    static dispatch_queue_t dwd_fluency_monitor_logs_queue;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dwd_fluency_monitor_logs_queue = dispatch_queue_create("com.dianwoda.DWDStastistics.dwd_fluency_monitor_logs_queue", NULL);
    });
    return dwd_fluency_monitor_logs_queue;
}

@interface DWDAppFluencyLogsFileManager : NSObject
...
@implementation DWDAppFluencyLogsFileManager
...
+ (void)writeLogsToLogDirectory:(NSString *)logs
{
    dispatch_async(dwd_fluency_monitor_logs_queue(), ^{
        NSData *logsData = [logs dataUsingEncoding:NSUTF8StringEncoding];
        if (logsData) {
            NSString *logFileName = [DWDAppFluencyLogsFileManager logFileName];
            NSString *filePath = [[DWDAppFluencyLogsFileManager logsDirectory] stringByAppendingPathComponent:logFileName];
...
@interface DWDAppFluencyMonitor () {
@private
...
@implementation DWDAppFluencyMonitor

static void runLoopObserverCallBack(CFRunLoopObserverRef observer,
                                    CFRunLoopActivity activity,
                                    void* info)
{
    DWDAppFluencyMonitor *appFluencyMonitor = (__bridge DWDAppFluencyMonitor*)info;
...
- (void)startMonitoring
{
    ...
    */
    ...
    // 在子线程监控时长
    dispatch_async(dwd_fluency_monitor_queue(), ^{
        while (YES)
        {
...
- (void)handleCallbacksStackForMainThreadStucked
{
    ...
    [DWDAppFluencyLogsFileManager writeLogsToLogDirectory:backtraceLogs];
}
...
- (NSString *)formatBacktraceLogsForAllThreads
{
    ...
    if (backTracesArray.count) {
        ...
        NSString *timeStamp = [DWDAppFluencyLogsFileManager timeStamp];
